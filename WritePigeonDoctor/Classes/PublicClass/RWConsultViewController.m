...
- (void)viewDidLoad
{
    ...
    [_requestManager obtainUserWithUserID:_order.payid];
}
...
- (void)requsetUser:(RWUserItem *)user responseMessage:(NSString *)responseMessage
{
    ...
    self.item = user;
    self.navigationItem.title = self.item.nickName;
    ...
    [self.baseManager addConsultHistoryWithItem:self.item
                                     completion:^(BOOL success)
     {
         if (!success)
         {
             [MBProgressHUD Message:@"消息缓存失败" For:weakSelf.view];
         }
     }];
}
