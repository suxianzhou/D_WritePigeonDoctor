...
- (void)reloadCellWithComment:(UMComComment *)comment
{
    ...
    if ([comment.creator.name isEqualToString:@"社区管理员"]) {
        
        self.nameLabel.text = comment.creator.name;
        
    }else
    {
        self.nameLabel.text = [comment.creator.name umName];
    }
