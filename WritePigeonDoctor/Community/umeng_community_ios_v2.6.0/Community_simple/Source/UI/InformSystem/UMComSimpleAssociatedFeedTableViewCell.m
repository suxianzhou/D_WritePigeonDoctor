...
- (void)awakeFromNib {
    ...
    self.cardView.layer.borderColor = UMComColorWithColorValueString(@"#dfdfdf").CGColor;
...
- (void)refreshCellWithFeed:(UMComFeed *)feed creator:(UMComUser *)creator time:(NSString *)time content:(NSString *)content checkWords:(NSArray *)checkWords
{
    ...
    if ([feed.creator.name isEqualToString:@"社区管理员"]) {
        ...
        self.creatorNameLabel.text = feed.creator.name;
        
    }else
    {
        self.creatorNameLabel.text = [feed.creator.name umName];
    }
    UMComMutiText *text = [UMComMutiText mutiTextWithSize:CGSizeMake(_commentLabel.frame.size.width, INT_MAX)
                                               ...
                                               checkWords:checkWords
                                                textColor:UMComColorWithColorValueString(@"#666666")
                                           highLightColor:UMComColorWithColorValueString(@"#469ef8")];
